...
if (GetPreviousPageName() != "annual_en_list")
    redir("annual_en_list.php");

$annual_id = request_num('edit_id');
if ($annual_id == '')
    redir("annual_en_list.php");
...
<div class="wrapper">
    ...
    <div class="content-wrapper">
        ...
        <section class="content">
            <div class="row">
                <div class="col-12">
                    <form name="edit_form" id="edit_form" action="annual_edit_ajax.php" data-toggle="validator" role="form">
                        <input type="hidden" id="annual_id" name="annual_id" value="<?= $id ?>">
...
<script>
    ...
    function ajaxCallback(status, form, json, options) {
        ...
        if (status === true) {
            if (json.result == true)
                success_brand_alert("Annual Report<?= $sub_title ?>成功", "", "javascript:history.go(-1)");
            else
                error_brand_alert("Annual Report<?= $sub_title ?>失敗", json.msg);
        } else {
            error_brand_alert("Annual Report<?= $sub_title ?>失敗", json.msg);
        }
    }
