...
if (GetPreviousPageName() != "business_list")
    redir("business_list.php");

$business_id = request_num('edit_id');
if ($business_id == '')
    redir("business_list.php");
...
<script>
    ...
    function ajaxCallback(status, form, json, options) {
        ...
        if (status === true) {
            if (json.result == true)
                success_brand_alert("業務簡介<?= $sub_title ?>成功", "", "javascript:history.go(-1)");
            else
                error_brand_alert("業務簡介<?= $sub_title ?>失敗", json.msg);
        } else {
            error_brand_alert("業務簡介<?= $sub_title ?>失敗", json.msg);
        }
    }
