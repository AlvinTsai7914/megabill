    ...
    <div class="content-wrapper">
                ...
                <div class="row mb-2">
                    <div class="col-sm-12">
                        <h1>Financial Highlights
                            <small>列表</small>
                            <button type="button" class="btn btn-sm btn-brand float-right" onclick="location.href = 'financial_en_edit.php?edit_id=0'"><span class="fas fa-plus"></span></button>
                        </h1>
                    </div>
                </div>
            </div>
        ...
        <section class="content">
            <div class="row">
                <div class="col-12">
                    <div class="card">
                        <form name="query_form" id="query_form" action="financial_en_list.php">
                            <div class="card-body">
                                <div class="row">
    ...
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">Financial Highlights<small>排序</small></h4>
...
<script>
    ...
    document.title += "-Financial Highlights";
    $(function () {
                ...
                dataSrc: function (json) {
                    ...
                    if (json.data != null) {
                        for (var i = 0; i < json.data.length; i++) {
                            ...
                            operate += "<button type='button' class='btn btn-sm btn-primary' onclick='redir(\"financial_en_edit.php?edit_id=" + row.id + "\")'><span class='fas fa-pen-square'></span></button>";
                            operate += "<button type='button' class='btn btn-sm btn-danger' onclick='delete_alert(\"Financial Highlights\"," + row.id + ",\"" + row.name + "\",\"<?= WEBAPI_FINANCIAL ?>\"); return false;'><span class='fas fa-trash-alt'></span></button>";
                            operate += "</div>";
    ...
    function reorder_modal_sumbit() {
        ...
        ids = ids.join(',');
        var jsondata = {category_id: '0', language_id: '1', ids: ids};
        var url = "financial_reorder_ajax.php";
        var result = new Object();
        if (AJAX(url, "POST", jsondata, result) === true) {
            var result = JSON.parse(result.data);
            if (result.result == true)
                success_brand_alert("Financial Highlights排序成功", "", "financial_en_list.php");
            else
                error_brand_alert("Financial Highlights排序失敗", result.msg);
        } else
            error_brand_alert("Financial Highlights排序失敗", result.msg);
    }
</script>
