...
<script>
    ...
    function ajaxCallback(status, form, json, options) {
        ...
        if (status === true) {
            if (json.result == true)
                success_brand_alert("買入中心工廠票券交易性商業本票<?= $sub_title ?>成功", "", "commercial_paper_edit.php");
            else
                error_brand_alert("買入中心工廠票券交易性商業本票<?= $sub_title ?>失敗", json.msg);
        } else {
            error_brand_alert("買入中心工廠票券交易性商業本票<?= $sub_title ?>失敗", json.msg);
        }
    }
