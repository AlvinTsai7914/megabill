...
$business_id = 4; //買入發行銀行承兌匯票及商業承兌匯票
$rows = null;
$row = null;
if ($business_id != 0) {
...
<script>
    ...
    function ajaxCallback(status, form, json, options) {
        ...
        if (status === true) {
            if (json.result == true)
                success_brand_alert("買入發行銀行承兌匯票及商業承兌匯票<?= $sub_title ?>成功", "", "acceptance_bill_edit.php");
            else
                error_brand_alert("買入發行銀行承兌匯票及商業承兌匯票<?= $sub_title ?>失敗", json.msg);
        } else {
            error_brand_alert("買入發行銀行承兌匯票及商業承兌匯票<?= $sub_title ?>失敗", json.msg);
        }
    }
