...
if (GetPreviousPageName() != "location_list")
    redir("location_list.php");
...
<script>
    ...
    function ajaxCallback(status, form, json, options) {
        ...
        if (status === true) {
            if (json.result == true)
                success_brand_alert("服務據點<?= $sub_title ?>成功", "", "javascript:history.go(-1)");
            else
                error_brand_alert("服務據點<?= $sub_title ?>失敗", json.msg);
        } else {
            error_brand_alert("服務據點<?= $sub_title ?>失敗", json.msg);
        }
    }
