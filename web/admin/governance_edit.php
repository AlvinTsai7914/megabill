...
if (GetPreviousPageName() != "governance_list")
    redir("governance_list.php");

$governance_id = request_num('edit_id');
if ($governance_id == '')
    redir("governance_list.php");
...
$row = null;
if ($governance_id != 0) {
    $url = WEBAPI_GOVERNANCE . "/" . $governance_id;
...
<div class="wrapper">
    ...
    <div class="content-wrapper">
        ...
        <section class="content">
            <div class="row">
                <div class="col-12">
                    <form name="edit_form" id="edit_form" action="governance_edit_ajax.php" data-toggle="validator" role="form">
                        <input type="hidden" id="governance_id" name="governance_id" value="<?= $id ?>">
                        <div class="card">
                            <div class="card-body">
                                <div class="row mb-2">
                                    ...
                                    <div class="col-md-5">
                                        <div class="input-group">
                                            <input type="text" id="file1url" name="file1url" class="form-control" value="<?= $file1url ?>" placeholder="請選擇檔案" readonly>
...
<script>
    ...
    function ajaxCallback(status, form, json, options) {
        ...
        if (status === true) {
            if (json.result == true)
                success_brand_alert("公司治理情形<?= $sub_title ?>成功", "", "javascript:history.go(-1)");
            else
                error_brand_alert("公司治理情形<?= $sub_title ?>失敗", json.msg);
        } else {
            error_brand_alert("公司治理情形<?= $sub_title ?>失敗", json.msg);
        }
    }
