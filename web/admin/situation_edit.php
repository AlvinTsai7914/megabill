...
if (GetPreviousPageName() != "situation_list")
    redir("situation_list.php");

$situation_id = request_num('edit_id');
if ($situation_id == '')
    redir("situation_list.php");
...
$row = null;
if ($situation_id != 0) {
    $url = WEBAPI_SITUATION . "/" . $situation_id;
...
<div class="wrapper">
    ...
    <div class="content-wrapper">
        ...
        <section class="content">
            <div class="row">
                <div class="col-12">
                    <form name="edit_form" id="edit_form" action="situation_edit_ajax.php" data-toggle="validator" role="form">
                        <input type="hidden" id="situation_id" name="situation_id" value="<?= $id ?>">
                        <div class="card">
                            <div class="card-body">
                                <div class="row mb-2">
...
<script>
    ...
    function ajaxCallback(status, form, json, options) {
        ...
        if (status === true) {
            if (json.result == true)
                success_brand_alert("年度執行情形<?= $sub_title ?>成功", "", "javascript:history.go(-1)");
            else
                error_brand_alert("年度執行情形<?= $sub_title ?>失敗", json.msg);
        } else {
            error_brand_alert("年度執行情形<?= $sub_title ?>失敗", json.msg);
        }
    }
