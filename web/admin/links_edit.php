...
if (GetPreviousPageName() != "links_list")
    redir("links_list.php");

$links_id = request_num('edit_id');
if ($links_id == '')
    redir("links_list.php");
...
<div class="wrapper">
    ...
    <div class="content-wrapper">
        ...
        <section class="content">
            <div class="row">
                <div class="col-12">
                    <form name="edit_form" id="edit_form" action="links_edit_ajax.php" data-toggle="validator" role="form">
                        <input type="hidden" id="links_id" name="links_id" value="<?= $id ?>">
                        <div class="card">
                            <div class="card-body">
                                <div class="row mb-2">
                                ...
                                <div class="row mb-2">
                                    ...
                                    <div class="col-md-5">
                                        <input type="text" id="link" name="link" class="form-control validate[required]" value="<?= $link ?>">
                                    </div>
...
<?php } ?>
    ...
    function ajaxCallback(status, form, json, options) {
        ...
        if (status === true) {
            if (json.result == true)
                success_brand_alert("友善連結<?= $sub_title ?>成功", "", "javascript:history.go(-1)");
            else
                error_brand_alert("友善連結<?= $sub_title ?>失敗", json.msg);
        } else {
            error_brand_alert("友善連結<?= $sub_title ?>失敗", json.msg);
        }
    }
