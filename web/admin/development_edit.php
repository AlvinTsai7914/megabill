...
if (GetPreviousPageName() != "development_list")
    redir("development_list.php");

$development_id = request_num('edit_id');
if ($development_id == '')
    redir("development_list.php");
...
$row = null;
if ($development_id != 0) {
    $url = WEBAPI_DEVELOPMENT . "/" . $development_id;
...
<div class="wrapper">
    ...
    <div class="content-wrapper">
        ...
        <section class="content">
            <div class="row">
                <div class="col-12">
                    <form name="edit_form" id="edit_form" action="development_edit_ajax.php" data-toggle="validator" role="form">
                        <input type="hidden" id="development_id" name="development_id" value="<?= $id ?>">
...
<script>
    ...
    function ajaxCallback(status, form, json, options) {
        ...
        if (status === true) {
            if (json.result == true)
                success_brand_alert("永續發展<?= $sub_title ?>成功", "", "javascript:history.go(-1)");
            else
                error_brand_alert("永續發展<?= $sub_title ?>失敗", json.msg);
        } else {
            error_brand_alert("永續發展<?= $sub_title ?>失敗", json.msg);
        }
    }
