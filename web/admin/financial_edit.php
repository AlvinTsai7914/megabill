                                ...
                                <div class="row mb-2">
                                    ...
                                    <div class="col-md-5">
                                        <select id="category_id" name="category_id" class="form-control select2" style="width: 100%">
                                            <?php
                                            foreach (is_array($financial_categorys) ? $financial_categorys : array() as $item) {
...
<script>
    ...
    function ajaxCallback(status, form, json, options) {
        ...
        if (status === true) {
            if (json.result == true)
                success_brand_alert("財務報告<?= $sub_title ?>成功", "", "javascript:history.go(-1)");
            else
                error_brand_alert("財務報告<?= $sub_title ?>失敗", json.msg);
        } else {
            error_brand_alert("財務報告<?= $sub_title ?>失敗", json.msg);
        }
    }
