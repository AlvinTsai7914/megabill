    ...
    <div class="content-wrapper">
            ...
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-12">
                        <h1>薪酬福利
                            <small>列表</small>
                            <button type="button" class="btn btn-sm btn-brand float-right" onclick="location.href = 'benefits_edit.php?edit_id=0'"><span class="fas fa-plus"></span></button>
                        </h1>
                    </div>
                </div>
            </div>
        ...
        <section class="content">
            <div class="row">
                <div class="col-12">
                    <div class="card">
                        <form name="query_form" id="query_form" action="benefits_list.php">
                            <div class="card-body">
                                <div class="row">
    ...
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">薪酬福利<small>排序</small></h4>
...
<script>
                ...
                dataSrc: function (json) {
                    ...
                    if (json.data != null) {
                        for (var i = 0; i < json.data.length; i++) {
                            ...
                            operate += "<button type='button' class='btn btn-sm btn-primary' onclick='redir(\"benefits_edit.php?edit_id=" + row.id + "\")'><span class='fas fa-pen-square'></span></button>";
                            operate += "<button type='button' class='btn btn-sm btn-danger' onclick='delete_alert(\"薪酬福利\"," + row.id + ",\"" + row.name + "\",\"<?= WEBAPI_BENEFITS ?>\"); return false;'><span class='fas fa-trash-alt'></span></button>";
                            operate += "</div>";
    ...
    function geturl()
    {
        var url = "_admin_list_ajax.php?api=<?= WEBAPI_BENEFITS ?>";
        var query_str = "1";
    ...
    function reorder_modal_sumbit() {
        ...
        var url = "benefits_reorder_ajax.php";
        var result = new Object();
        if (AJAX(url, "POST", jsondata, result) === true) {
            var result = JSON.parse(result.data);
            if (result.result == true)
                success_brand_alert("薪酬福利排序成功", "", "benefits_list.php");
            else
                error_brand_alert("薪酬福利排序失敗", result.msg);
        } else
            error_brand_alert("薪酬福利排序失敗", result.msg);
    }
</script>
