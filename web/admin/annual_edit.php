...
if (GetPreviousPageName() != "annual_list")
    redir("annual_list.php");
...
<script>
    ...
    function ajaxCallback(status, form, json, options) {
        ...
        if (status === true) {
            if (json.result == true)
                success_brand_alert("年報<?= $sub_title ?>成功", "", "javascript:history.go(-1)");
            else
                error_brand_alert("年報<?= $sub_title ?>失敗", json.msg);
        } else {
            error_brand_alert("年報<?= $sub_title ?>失敗", json.msg);
        }
    }
