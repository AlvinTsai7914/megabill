    ...
    <div class="content-wrapper">
            ...
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-12">
                        <h1>Annual Report
                            <small>列表</small>
                            <button type="button" class="btn btn-sm btn-brand float-right" onclick="location.href = 'annual_en_edit.php?edit_id=0'"><span class="fas fa-plus"></span></button>
                        </h1>
                    </div>
                </div>
            </div>
        ...
        <section class="content">
            <div class="row">
                <div class="col-12">
                    <div class="card">
                        <form name="query_form" id="query_form" action="annual_en_list.php">
                            <div class="card-body">
                                <div class="row">
    ...
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">Annual Report<small>排序</small></h4>
...
<script>
                ...
                dataSrc: function (json) {
                    ...
                    if (json.data != null) {
                        for (var i = 0; i < json.data.length; i++) {
                            ...
                            operate += "<button type='button' class='btn btn-sm btn-primary' onclick='redir(\"annual_en_edit.php?edit_id=" + row.id + "\")'><span class='fas fa-pen-square'></span></button>";
                            operate += "<button type='button' class='btn btn-sm btn-danger' onclick='delete_alert(\"Annual Report\"," + row.id + ",\"" + row.name + "\",\"<?= WEBAPI_ANNUAL ?>\"); return false;'><span class='fas fa-trash-alt'></span></button>";
                            operate += "</div>";
    ...
    function geturl()
    {
        var url = "_admin_list_ajax.php?api=<?= WEBAPI_ANNUAL ?>";
        var query_str = "1";
    ...
    function reorder_modal_sumbit() {
        ...
        ids = ids.join(',');
        var jsondata = {language_id: '1', ids: ids};
        var url = "annual_reorder_ajax.php";
        var result = new Object();
        if (AJAX(url, "POST", jsondata, result) === true) {
            var result = JSON.parse(result.data);
            if (result.result == true)
                success_brand_alert("Annual Report排序成功", "", "annual_en_list.php");
            else
                error_brand_alert("Annual Report排序失敗", result.msg);
        } else
            error_brand_alert("Annual Report排序失敗", result.msg);
    }
</script>
