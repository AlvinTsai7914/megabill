...
if (GetPreviousPageName() != "propaganda_list")
    redir("propaganda_list.php");

$propaganda_id = request_num('edit_id');
if ($propaganda_id == '')
    redir("propaganda_list.php");
...
$row = null;
if ($propaganda_id != 0) {
    $url = WEBAPI_PROPAGANDA . "/" . $propaganda_id;
...
<div class="wrapper">
    ...
    <div class="content-wrapper">
        ...
        <section class="content">
            <div class="row">
                <div class="col-12">
                    <form name="edit_form" id="edit_form" action="propaganda_edit_ajax.php" data-toggle="validator" role="form">
                        <input type="hidden" id="propaganda_id" name="propaganda_id" value="<?= $id ?>">
                        <div class="card">
                            <div class="card-body">
                                <div class="row mb-2">
...
<?php } ?>
    ...
    function ajaxCallback(status, form, json, options) {
        ...
        if (status === true) {
            if (json.result == true)
                success_brand_alert("政策宣導<?= $sub_title ?>成功", "", "javascript:history.go(-1)");
            else
                error_brand_alert("政策宣導<?= $sub_title ?>失敗", json.msg);
        } else {
            error_brand_alert("政策宣導<?= $sub_title ?>失敗", json.msg);
        }
    }
