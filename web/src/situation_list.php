...
$url = WEBAPI_SITUATION . "?count=true";
if (CallAPI('GET', $url, null, null, $resultMessage, $situation_count) != 200) {
    $situation_count = null;
}
$total_pages = max(ceil($situation_count / $page_length), 1);
...
if (CallAPI('GET', $url, null, null, $resultMessage, $situations) != 200) {
    $situations = null;
}
?>
<div class="wrapper">
    ...
    <main class="pg company_list">
        <div class="container">
            <ul class="decor_lists__t">
                <?php foreach (is_array($situations) ? $situations : array() as $item) { ?>
                    <li><a href="<?= BASE_PATH ?>/situation_detail/<?= $item["id"] ?>"><?= $item["name"] ?></a></li>
                <?php } ?>
            </ul>
            <?php if ($total_pages > 1) { ?>
                ...
                <ul class="pgs_list">
                    <li><a href="<?= BASE_PATH ?>/situation_list/<?= max($g_page_index - 1, 1); ?>"><img src="<?= BASE_PATH ?>/src/img/icon/angle_left.png" alt="上一頁"></a></li>
                    <?php for ($i = 1; $i <= $total_pages; $i++) { ?>
                        <?php if ($i == $min_index && $i > 2) { ?>
                            ...
                        <?php } ?>
                        <?php if ($i == 1 || ($i >= $min_index && $i <= $max_index) || $i == $total_pages) { ?>
                            <li class="<?= ($i == $g_page_index) ? "current" : "" ?>"><a href="<?= BASE_PATH ?>/situation_list/<?= $i ?>"><?= $i ?></a></li>
                        <?php } ?>
                        <?php if ($i == $max_index && $i < ($total_pages - 1)) { ?>
                            ...
                        <?php } ?>
                    <?php } ?>
                    <li><a href="<?= BASE_PATH ?>/situation_list/<?= min($g_page_index + 1, $total_pages); ?>"><img src="<?= BASE_PATH ?>/src/img/icon/angle_right.png" alt="下一頁"></a></li>
                </ul>
            <?php } ?>
        </div>
    </main>
